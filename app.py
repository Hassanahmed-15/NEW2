...
def save_report():
    ...
    # Helper function to safely load JSON data from the form
...
def final_report():
    ...
    # Clean up the data to remove similarity scores and brackets
    # for key in ['mitraclip', 'impella', 'severe_lv_dilation', 'moderate_lv_dilation', 'mild_lv_dilation', 'sig_elev_ra_pressure', 'norm_r_atrial_pressure']:
    #     report_data[key] = [item[0] if isinstance(item, list) else item for item in report_data[key]]
...
def process_video(video_path):
    ...
    echo_clip, _, preprocess_val = create_model_and_transforms("hf-hub:mkaichristensen/echo-clip", precision="bf16", device="cuda")
    ...
    test_video = test_video[0:min(40, len(test_video)):2].cuda()
    ...
    def find_top_n_matching_prompts(video_embedding, prompt_key, top_n=3):
        ...
        tokenized_prompts = tokenize(prompts).cuda()
        with torch.no_grad():
        ...
        top_n_indices = similarities.topk(top_n, largest=True).indices.cpu().detach().numpy().flatten()
        top_n_similarities = similarities.topk(top_n, largest=True).values.cpu().detach().to(torch.float32).numpy().flatten()
        top_n_prompts = [prompts[idx] for idx in top_n_indices]
        top_n_results = list(zip(top_n_prompts, top_n_similarities))
    ...
    def find_top_cont_prompts(t_video_embedding, key, value_range):
        ...
        gen_prompts = tokenize(gen_prompts).cuda()
        with torch.no_grad():
            encoded_gen_prompts = echo_clip.encode_text(gen_prompts)
            norm_gen_prompts = F.normalize(encoded_gen_prompts, dim=-1)
        predicted_value = compute_regression_metric(t_video_embedding, norm_gen_prompts, prompt_values)
        closest_prompt_index = (torch.abs(torch.tensor(prompt_values, device='cuda') - predicted_value.item())).argmin()
        closest_prompt_text = gen_prompts[closest_prompt_index].cpu().numpy()
    ...
    bins = ["severe_right_ventricle_size", "moderate_right_ventricle_size", "mild_right_ventricle_size",
    ...
    for b in bins:
        ...
        prompt = tokenize(bin_prompt).cuda()
        prompt_embeddings = F.normalize(echo_clip.encode_text(prompt), dim=-1)
        prompt_predictions = compute_binary_metric(test_video_embedding, prompt_embeddings)
        f1_calibrated_threshold = 0.28
        if prompt_predictions.item() > f1_calibrated_threshold:
